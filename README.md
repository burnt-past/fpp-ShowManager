...
- **Website link** — serve a small JSON feed of tonight's show times and a status banner to an external site. The site fetches it **server‑to‑server** through a tunnel (visitors never touch the box), guarded by an **unguessable key** that never reaches a browser; a **push‑to‑static‑host** mode is also available. Show times only — no settings or secrets — and no hostnames or credentials are baked into the plugin. See [docs/website-integration.md](docs/website-integration.md).
